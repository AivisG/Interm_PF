...
class TimeSeriesTransformer(tf.keras.Model):
    def __init__(self, num_layers, d_model, num_heads, dff, input_seq_len, target_seq_len, dropout_rate=0.1):
        super(TimeSeriesTransformer, self).__init__()
        
        self.encoder = TransformerEncoder(num_layers, d_model, num_heads, dff, input_seq_len, dropout_rate)
        self.final_layer = tf.keras.layers.Dense(1)
        
    def call(self, inputs, training=False):
        enc_output = self.encoder(inputs, training=training)  # <-- Fix applied here
        output = self.final_layer(enc_output[:, -1, :])  # Predict next timestep
        return output
